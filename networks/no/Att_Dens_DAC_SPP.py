...
class DACblock(nn.Module):
    def __init__(self, channel):
        ...
        self.dilate1 = nn.Conv2d(channel, channel, kernel_size=3, dilation=3, padding=3)
    ...
    def forward(self, x):
        ...
        dilate2_out = nonlinearity(self.dilate1(self.conv1x1(x)))
        dilate3_out = nonlinearity(self.dilate1(self.conv3x3(self.conv1x1(x))))
        dilate4_out = nonlinearity(self.dilate1(self.conv5x5(self.conv1x1(x))))
        dilate5_out = nonlinearity(self.dilate2(self.dilate1(self.conv3x3(self.conv1x1(x)))))
...
class CE_Net_(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder
        d3 = self.decoder4(e3)
        e3 = self.Att4(g=d3,x=e2)
        d3 =  torch.cat((e3,d3),dim=1)
        d3 = self.Conv4(d3)

        d2 = self.decoder3(d3)
        e2 = self.Att3(g=d2, x=e1)
        d2 = torch.cat((e2, d2), dim=1)
        d2 = self.Conv3(d2)


        d1 = self.decoder2(d2)
        x1 = self.Att2(g=d1, x=x)
