...
class DACblock(nn.Module):
    ...
    def forward(self, x):
        dilate1_out = nonlinearity(self.conv3x3(self.conv1x1(x)))
        dilate2_out = nonlinearity(self.dilate2(self.conv3x3(self.conv1x1(x))))
        dilate3_out = nonlinearity(self.dilate3(self.dilate2(self.conv5x5(self.conv1x1(x)))))
        dilate4_out = self.conv1x1(x)
...
class SPPblock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.pool4 = nn.MaxPool2d(kernel_size=[7, 7], stride=7)
...
class DeconveBlock(nn.Module):
    def __init__(self, in_channels, n_filters):
        super(DeconveBlock, self).__init__()
...
class CE_Net_(nn.Module):
    def __init__(self, num_classes=Constants.BINARY_CLASS, num_channels=3):
        ...
        self.decoder4 = DeconveBlock(1028, filters[3])
        self.Att4 = Attention_block(F_g=filters[3], F_l=filters[3], F_int=filters[2])
        self.Conv4 = ConvBlock(1024,filters[3])
        ...
        self.Conv2 = ConvBlock(filters[2], filters[1])

        self.decoder1 = DecoderBlock(filters[1], filters[0])
        self.Att1 = Attention_block(F_g=filters[0], F_l=filters[0], F_int=int(32))
        self.Conv1 = ConvBlock(filters[1], filters[0])
    ...
    def forward(self, x):
        ...
        e1 = self.encoder1(x)
        e1 = self.transition1(e1)
        e2 = self.encoder2(e1)
        e2 = self.transition2(e2)
        e3 = self.encoder3(e2)
        e3 = self.transition3(e3)
        e4 = self.encoder4(e3)
        e4 = self.norm5(e4)
        ...
        # Decoder
        d4 = self.decoder4(e4)
        e4 = self.Att4(g=d4,x=e3)
        d4 = torch.cat((e4, d4), dim=1)
        d4 = self.Conv4(d4)

        d3 = self.decoder3(d4)
        e3 = self.Att3(g=d3, x=e2)
        d3 = torch.cat((e3, d3), dim=1)
        d3 = self.Conv3(d3)


        d2 = self.decoder2(d3)
        e2 = self.Att2(g=d2, x=e1)
        d2 = torch.cat((e2, d2), dim=1)
        d2 = self.Conv2(d2)

        d1 = self.decoder1(d2)
        e1 = self.Att1(g=d1, x=x)
