...
class CE_Net_(nn.Module):
    def __init__(self, num_classes=Constants.BINARY_CLASS, num_channels=3):
        ...
        self.Conv5 = ConvBlock(516, filters[3])
    ...
    def forward(self, x):
        ...
        # Decoder
        d4 = self.Att4(g=e6,x=e4)
        #d4 =  torch.cat((e6,e4),dim=1)
        d4 = self.Conv4(d4)
        d4 = self.decoder4(d4+e6)

        d3 = self.Att3(g=d4, x=e3)
        #d3 = torch.cat((d4, e3), dim=1)
        d3 = self.Conv3(d3)
        d3 = self.decoder3(d3 + d4)

        d2 = self.Att2(g=d3, x=e2)
        #d2 = torch.cat((d3, e2), dim=1)
        d2 = self.Conv2(d2)
        d2 = self.decoder2(d2 + d3)

        d1 = self.Att1(g=d2, x=e1)
        #d1 = torch.cat((d2, e1), dim=1)
        d1 = self.Conv1(d1)
        d1 = self.decoder1(d1 + d2)
