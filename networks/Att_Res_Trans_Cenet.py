...
from networks.modeling import VisionTransformer

import networks.configs  as config
...
class CE_Net_(nn.Module):
    def __init__(self, num_classes=Constants.BINARY_CLASS, num_channels=3):
        ...
        #self.trans1 = VisionTransformer(img_size=112, channels=256)
        self.encoder2 = model.layer2
        #self.trans2 = VisionTransformer(img_size=56, channels=512)
        self.encoder3 = model.layer3
        self.trans3 = VisionTransformer(img_size=28, channels=1024)
        self.encoder4 = model.layer4

        self.dblock = DACblock(2048)

        self.trans4 = VisionTransformer(img_size=14, channels=2048)
    ...
    def forward(self, x):
        ...
        e1 = self.encoder1(x)
        #e1 = self.trans1(self.encoder1(x))
        e2 = self.encoder2(e1)
        #e3 = self.encoder3(e2)
        #e2 = self.trans2(self.encoder2(e1))
        e3 = self.trans3(self.encoder3(e2))

        e4 = self.encoder4(e3)

        # Center
        e4 = self.trans4(self.dblock(e4))
