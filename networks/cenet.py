...
# from visualize_feature_map import draw_features_plot
# from visualize_feature_map import show_feature_map
...
class UNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=4): #我修改的地方
    ...
    def forward(self, x):
        ...
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        # show_feature_map(x.detach())
        x = self.outc(x)
        # draw_features_plot(x.detach())
        x = F.sigmoid(x)
        #x = self.relu(x)
        return x
